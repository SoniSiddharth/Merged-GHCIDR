...
def saveAsPickle(dataset, path):   
    # location = '/content/drive/MyDrive/ML project/Dataset Pickle/{}_{}.pickle'.format(self.variantName,self.datasetName)     
    dbfile = open(path, 'wb') 
    pickle.dump(dataset, dbfile)
    print("Saved at: "+ path)                      
    dbfile.close()

def checkMergedClusters(args):
    if "MergedClusters" not in os.listdir():
        return False
    path = args.datasetName + "_" + str(args.beta) +'.pickle' 
    if path not in os.listdir("MergedClusters"):
        return False
    return True
...
def getTinyImagenetData(path, mode=0):
    """
        returns preprocessed MNIST dataset
    """
    # dbfile = open("")
    X_train, y_train, X_test, y_test = pickle.load(path, 'rb')
    X_train = X_train.reshape(100000, 12288).astype('float32') # reshape 60,000 28 x 28 matrices into 60,000 784-length vectors.
    # X_test = X_test.reshape(10000, 12288).astype('float32')   # reshape 10,000 28 x 28 matrices into 10,000 784-length vectors.
    X_test = np.array(X_test,dtype="float32")
    X_train /= 255                        # normalize each value for each pixel for the entire vector for each input
    X_test /= 255
    uniqueClasses = 100
